...
def spec_augment(image, label):
    ...
    for _ in range(2):
        f  = tf.random.uniform((), 0, 20, dtype=tf.int32)
        f0 = tf.random.uniform((), 0, IMG_SIZE[0] - f, dtype=tf.int32)
        mask = tf.concat([
            tf.ones([f0, IMG_SIZE[1], 3]),
            tf.zeros([f, IMG_SIZE[1], 3]),
            tf.ones([IMG_SIZE[0] - f0 - f, IMG_SIZE[1], 3])
        ], axis=0)
        img = img * mask
    for _ in range(2):
        t  = tf.random.uniform((), 0, 40, dtype=tf.int32)
        t0 = tf.random.uniform((), 0, IMG_SIZE[1] - t, dtype=tf.int32)
        mask = tf.concat([
            tf.ones([IMG_SIZE[0], t0, 3]),
            tf.zeros([IMG_SIZE[0], t, 3]),
            tf.ones([IMG_SIZE[0], IMG_SIZE[1] - t0 - t, 3])
        ], axis=1)
        img = img * mask
    return img, label
...
# ============================================================
# PIPELINE
# cache() before augmentation so fresh augment every epoch
# No manual normalization — EfficientNet handles it internally
# ============================================================
...
# ============================================================
# MODEL — EfficientNetB0 (same as v3, proven to work)
# ============================================================
...
print(f"\nFine-tuning {sum(1 for l in base_model.layers if l.trainable)} layers")

# Add this block just before the Phase 2 model.compile()
print("Enabling SpecAugment for Phase 2...")
train_ds = (train_ds_raw
            .cache()
...
model.save("final_covidcoughnet_v5.keras")
print("\n✅ Final model saved → final_covidcoughnet_v5.keras")
...
sns.heatmap(cm, annot=True, fmt='d', cmap='Blues',
            ...
            linewidths=0.5)
plt.title("Confusion Matrix — CovidCoughNet v5", fontsize=13)
plt.ylabel("True Label")
plt.xlabel("Predicted Label")
plt.tight_layout()
plt.savefig("confusion_matrix_v5.png", dpi=150)
plt.show()
...
plot_metric("accuracy", "Accuracy — v5", "curve_accuracy_v5.png")
plot_metric("loss",     "Loss — v5",     "curve_loss_v5.png")

print("\n✅ Done. Check confusion_matrix_v5.png and classification report above.")